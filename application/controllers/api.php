...
//                                        );
                            ...
                            if ($req === false) {
                                ...
                                $data = array(
                                    ...
                                    'region' => $obj->region ? $obj->region : 1,
...
//                $inspection_id = 14977;
    ...
    {
        error_log("---------------- send report function started!----------------");
        $ret = false;
        if ($manual_report) {
            $inspection = $this->utility_model->get('ins_inspection', array('id' => $inspection_id));
        } else {
        ...
        error_log("manual_report:" . $manual_report);

        error_log('data ===========> inspection');
        error_log(json_encode($inspection));
...
//                unlink($file2);
            ...
            switch ($inspection['type']) {
                ...
                case 2:
                    ...
                    break;
                case 5:
                    $html_subject = "Stucco Inspection";
                    break;
            }
...
//            $google_map = "<img width='300' src='http://maps.googleapis.com/maps/api/staticmap?center=" . $inspection['latitude'] . "+" . $inspection['longitude'] . "&zoom=16&scale=false&size=300x300&maptype=roadmap&format=jpg&visual_refresh=true' alt='Google Map'>";
            ...
            $google_map = "<img class='img-responsive for-preview google-map' data-src='http://maps.googleapis.com/maps/api/staticmap?center=" . $inspection['latitude'] . "+" . $inspection['longitude'] . "&zoom=15&scale=false&size=750x750&maptype=roadmap&format=jpg&visual_refresh=true&key=" . GOOGLE_MAP_KEY . "' src='http://maps.googleapis.com/maps/api/staticmap?center=" . $inspection['latitude'] . "+" . $inspection['longitude'] . "&zoom=16&scale=false&size=300x300&maptype=roadmap&format=jpg&visual_refresh=true&key=" . GOOGLE_MAP_KEY . "' alt='Google Map'>";

            //error_log("image URL ===> " . $google_map);
 ...
 style='border:none;width: 24%;' valign='bottom'>Number of stories :</td>
    ...
    word-break: break-word;'>" . $inspection['emp_per_inspection'] . "</td>
        ...
        if (count($image_int_inspection) > 0) {
            ...
            <img src='" . $item['upload_link'] . $item['stucco_value_src'] . "' style='width: auto; margin-top:10px; height: 611px;  text-align: center;'>
            </li>";
            }
            $newHtml .= "</ul></div>";
        }
      ...
      WS = Weep Screed Condition</p>
       ...
       <div><h1 class='book_page' style='text-align: center;'>Front Elevation</h1></div>";
        if (!empty($inspection['image_front_building'])) {
            $newHtml .= "<div style='text-align: center;' class='pdfimage'><img height='611px' src='" . "resource/upload/stucco/front/" . $inspection['image_front_building'] . "' style='width: auto; height: 611px;  text-align: center;'></div>";
        }
        $newHtml .= "<div>
...
<table style='text-align: left;border:none;border: none;border-collapse: separate;border-spacing: 0 30px;'><tr><td style='width: 28%; '>If Yes, what evidence of intrusion? </td><td style='width: 51%; border-bottom: 1px solid;'>" . $inspection_images['text_front_building4'] . "</td></tr></table>
   ...
   <div><h1 class='book_page' style='text-align: center;'>Right Elevation</h1></div>";
        ...
        if (!empty($inspection['image_right_building'])) {
            $newHtml .= "<div style='text-align: center;' class='pdfimage'><img height='611px' src='resource/upload/stucco/right/" . $inspection['image_right_building'] . "' style='width: auto; height: 611px;  text-align: center;'></div>";
        }
...
<table style='text-align: left;border:none;border: none;border-collapse: separate;border-spacing: 0 30px;'><tr><td style='width: 60%; '>If Yes, approximately how many cracks per 10 linear feet? </td><td style='width: 51%; border-bottom: 1px solid;'>" . $inspection_images['text_right_building2'] . "</td></tr></table>
     ...
     <p style='line-height:20px;padding-left:20px;'><span style='border-bottom: 1px solid;'>" . $item['stucco_comments'] . "</span></p>
        ...
        if (!empty($inspection['image_back_building'])) {
            $newHtml .= "<div style='text-align: center;' class='pdfimage'><img height='611px' src='resource/upload/stucco/back/" . $inspection['image_back_building'] . "' style='width: auto; height: 611px;  text-align: center;'></div>";
        }
     ...
     <p style='line-height:20px;padding-left:20px;'><span style='border-bottom: 1px solid;'>" . $item['stucco_comments'] . "</span></p>
      ...
      <div><h1 class='book_page' style='text-align: center;'>Left Elevation</h1></div>";
        if (!empty($inspection['image_left_building'])) {
            $newHtml .= "<div style='text-align: center;' class='pdfimage'><img height='611px' src='resource/upload/stucco/left/" . $inspection['image_left_building'] . "' style='width: auto; height: 611px;  text-align: center;'></div>";
        }
   ...
   <div style='margin: 30px 0 20px;' class='signimage'>
     <p style='line-height:25px;padding-left:20px;'>Inspector’s Signature <span style='border-bottom: 1px solid;'>";
        if (!empty($inspection['image_signature'])) {
            $newHtml .= "<div style='text-align: left;' class='pdfimagesign'><img width='250px' src='resource/upload/stucco/signature/" . $inspection['image_signature'] . "' style='width: 150px;'></div>";
        }
...
//        }
        ...
        try {
            if ($mail->send()) {
                ...
                error_log("mail_send failed exception=======>" . $mail->ErrorInfo);
                return $mail->ErrorInfo;
            }

        } catch (Exception $e) {
            error_log("mail_send catch exception============>" . $e);
        }
...
//        }
    ...
    private function get_report_data__for_statistics_inspection($region, $community, $start_date, $end_date, $status, $type, $is_array = false, $include_description = true, $table_order = array())
    {
            ...
            foreach ($sub_types as $sub_type) {
                ...
                if ($sub_inspection != "Energy") {
                    $sub_sql = " SELECT c.name AS result_name, t.result_code, t.tnt "
                        . " FROM ins_code c, ( select a.result_code, count(*) as tnt from ( $sql and a.type in ($sub_type)) a group by a.result_code ) t "
                    ...
                    if ($tnt && is_array($tnt)) {
                        ...
                        $count_text .= "<h4 class='row sub-inspection'>" . $sub_inspection . " : <span class='total-0'>Total: " . $sub_total . "</span>";
                        ...
                        foreach ($tnt as $row) {
                            ...
                            if ($sub_inspection == 'Stucco') {
                                $count_text .= '<span class="label-warning" >';
                                $count_text .= "NONE: " . $row['tnt'];
                            } else {
                                if ($row['result_code'] == '1') {
                                    $count_text .= '<span class="label-success">';
                                } else if ($row['result_code'] == '3') {
                                    $count_text .= '<span class="label-danger">';
                                } else {
                                    $count_text .= '<span class="label-warning">';
                                }
                ...
                } else if ($sub_inspection == "Energy") {
                    $sub_sql = " SELECT c.name AS result_name, t.result_code, t.tnt, t.result_duct_leakage, t.result_envelop_leakage"
                        . " FROM ins_code c, ( select a.result_code, count(*) as tnt, a.result_duct_leakage, a.result_envelop_leakage from ( $sql and a.type in ($sub_type) ) a group by a.result_code, a.result_duct_leakage, a.result_envelop_leakage ) t "
                        . " WHERE c.kind='rst' AND c.code=t.result_code ORDER BY c.code ";
                    ...
                    if ($tnt && is_array($tnt)) {
                        ...
                        foreach ($tnt as $row) {
                            ...
                            if ($row['result_duct_leakage'] == '1') {
                                $count_text .= '<span class="label-success">';
                                $count_text .= "Duct Pass: " . $row['tnt'];
                            } else {
                                $count_text .= '<span class="label-danger">';
                                $count_text .= "Duct Fail: " . $row['tnt'];
                            }
                            if ($sub_total != 0) {
                                $tnt = intval($row['tnt']);
                                $count_text .= "(" . round($tnt * 1.0 / $sub_total * 100, 2) . "%)";
                            ...
                            if ($row['result_envelop_leakage'] == '1') {
                                $count_text .= '<span class="label-success">';
                                $count_text .= "Envelop Pass: " . $row['tnt'];
                            } else {
                                $count_text .= '<span class="label-danger">';
                                $count_text .= "Envelop Fail: " . $row['tnt'];
                            }
                            if ($sub_total != 0) {
                                $tnt = intval($row['tnt']);
                                $count_text .= "(" . round($tnt * 1.0 / $sub_total * 100, 2) . "%)";
        ...
        if ($region != "") {
            if (gettype($region) == 'string') {
                if (strpos($region, ',') !== false) {
                    $rows = $this->utility_model->get_list__by_sql('select * from ins_region where id in (' . $region . ')');
                    if ($rows) {
                        $regions = [];
                        foreach ($rows as $row) {
                            array_push($regions, $row['region']);
                        }
                        $sub_title .= join(" | ", $regions);
                    }
                } else {
                ...
                foreach ($region as $reg) {
                    $region_str = $region_str . $reg . ",";
                }
                if (strlen($region_str) > 0) {
                    $region_str = substr($region_str, 0, strlen($region_str) - 1);
                    $region_str = "(" . $region_str . ")";
                }

                $rows = $this->utility_model->get_list__by_sql('select * from ins_region where id in ' . $region_str);
                if ($rows) {
                    $regions = [];
                    foreach ($rows as $row) {
                        array_push($regions, $row['region']);
        ...
        if ($data && is_array($data)) {
            foreach ($data as $row) {
                ...
                if ($row['result_code'] == '0') {
                    if ($row['type'] == '5') {
                        $html_body .= '<td class="text-center"><span class="label-warning">None</span>';
                    } else {
                        $value = "";
                        if ($row['result_duct_leakage'] == '1') {
                            $cls = "label-success";
                        ...
                        $html_body .= '<td class="text-center"><span class="label ' . $cls . '">Duct: ' . $value . '</span><br>';
                        ...
                        $html_body .= '<span class="label ' . $cls . '">Envelop: ' . $value . '</span></td>';
                    }
                } else {
                    if ($row['result_code'] == '1') {
                        $cls = "label-success";
                ...
                if ($include_description) {
                    array_push($reports, array(
                        ...
                        'date' => $row['start_date'],
                        'result' => $row['result_code'] != '0' ? $row['result_name'] : ($row['type'] == '5' ? 'None' : ('Duct:' . ($row['result_duct_leakage'] == '1' ? 'Pass' : 'Fail') . ' Envelop: ' . ($row['result_envelop_leakage'] == '1' ? 'Pass' : 'Fail'))),
        ...
        $html_body .= '</tbody>';
        $html_body .= '</table>';
        // $html_body .= '<h3>select * from ins_region where id in (' . $region . ')</h3>';
        $html_body .= '</div>';
...
//            'image_left_building' => $this->input->post($image_left_building),
        ...
        if (!empty($inspection_id)) {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                if ($insert_inspection_images && $update_requested_inspection) {
                    ...
                    $this->send_report($user_id, $inspection_id);
    ...
    public function create_zip($communityId, $type = "full")
    {
        //for full error log from server
        //error_reporting(E_ALL);
        ini_set('memory_limit', '2048M');

        $response = array('code' => -1, 'message' => 'Failed to download', 'drainage_count' => 0, 'lath_count' => 0, 'zip_url_1' => "", 'zip_url_2' => "", 'html' => []);
        if ($this->session->userdata('user_id')) {
            if ($communityId === false || $communityId == "") {
                $response['message'] = "Invalid Community";
            } else {
                //$communityId = str_replace("%20", " ", $communityId);
                if ($type != "pass") $type = "full";

                $uu_id = $this->uuid->v4();
                $zip_filename = 'resource/upload/report/water_intrusion_drainageplan_community_' . $communityId . '_' . $uu_id . '.zip';
                $drainage_count = 0;

                // query for drainage plan inspection
                $sql = "select * from ins_inspection where community = '$communityId' and type = '1' and region <> '0'";
                $inspections = $this->utility_model->get_list__by_sql($sql);

                if ($inspections) {
                    $zip1 = new ZipArchive();

                    if ($zip1->open($zip_filename, ZipArchive::CREATE) === true) {

                        foreach ($inspections as $inspection) {
                            try {
                                //var_dump('zip_drainage_inspection' . $inspection['id']);
                                $html = $this->get_report_html($inspection['id'], $type);
                                // array_push($response['html'], $html);
                                $this->m_pdf->initialize();
                                $this->m_pdf->pdf->WriteHTML($html);
                                $uu_id1 = $this->uuid->v4();
                                $filepath = "resource/upload/report/";
                                $filename = "report_id_" . $inspection['id'] . '_' . $uu_id1 . ".pdf";

                                $this->m_pdf->pdf->Output($filepath . $filename, "F");

                                // Add report to the zip file inside folder
                                $zip1->addFile($filepath . $filename, $filename);

                                $drainage_count++;
                            } catch (Exception $exception) {
                                error_log('api:create_zip:drainage: ========>' . $exception->getMessage());
                            }
                        }
                        // set response
                        if ($drainage_count > 0) {
                            $response['drainage_count'] = $drainage_count;
                            $response['code'] = 0;
                            $response['message'] = "Zip file was created successfully.";
                            $response['zip_url_1'] = $zip_filename;
                        } else {
                            $response['message'] = "No Drainage Plan Inspection was found.";
                        }
                ...
                $zip_filename = 'resource/upload/report/water_intrusion_lath_community_' . $communityId . '_' . $uu_id . '.zip';
                $lath_count = 0;

                // query for Lath inspection
                $sql = "select * from ins_inspection where community = '$communityId' and type = '2' and region <> '0'";
                $inspections = $this->utility_model->get_list__by_sql($sql);

                if ($inspections) {
                    $zip2 = new ZipArchive();

                    if ($zip2->open($zip_filename, ZipArchive::CREATE) === true) {
                        foreach ($inspections as $inspection) {
                            try {
                                //var_dump('zip_lath_inspection' . $inspection['id']);
                                ...
                                $uu_id2 = $this->uuid->v4();
                                $filepath = "resource/upload/report/";
                                $filename = "report_id_" . $inspection['id'] . '_' . $uu_id2 . ".pdf";
                                $this->m_pdf->pdf->Output($filepath . $filename, "F");

                                // Add report to the zip file inside folder
                                $zip2->addFile($filepath . $filename, $filename);

                                $lath_count++;
                            } catch (Exception $exception) {
                                error_log('api:create_zip:lath: ========>' . $exception->getMessage());
                            }
                        }
                        // set response
                        if ($lath_count > 0) {
                            $response['lath_count'] = $lath_count;
                            $response['zip_url_2'] = $zip_filename;
                        } else {
                            $response['message'] = "No Lath Inspection was found.";
                        }
                ...
                // set response
                if ($drainage_count > 0 || $lath_count > 0) {
                    $response['code'] = 0;
                    $response['message'] = "Zip file was created successfully.";
                } else {
                    $response['message'] = "No Drainage Plan and Lath Inspection was found.";
                }
