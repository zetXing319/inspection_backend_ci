...
class Utility_model extends CI_Model    {
    ...
    function fetch_data($query)
 {
   ...
   $regioncountsql ="select * from ins_region WHERE id = '".$result[0]['region']."' limit 1";  

    $regioncountrun = $this->db->query($regioncountsql);
    $resultregion= $regioncountrun->result_array();   
          return $result12=array(
                         "community_name"=>$result[0]['community_id'],
                         "community_id"=>$result[0]['community_name'],
                         "city"=>$result[0]['city'],
                         "region_name"=>$resultregion[0]['region'],
                         "region"=>$resultregion[0]['id'],
 ...
 $offercountsql ="select address,job_number from ins_building WHERE ".$extra." job_number  like '" . $job_number ."%' UNION select address,job_number from ins_building   where  ".$extra." job_number  like '" . $job_number ."%'";
    $offercountrun = $this->db->query($offercountsql);
